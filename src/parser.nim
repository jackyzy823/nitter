...
proc parseTweet(js: JsonNode): Tweet =
  ...
  with jsCard, js{"card"}:
    ...
    if "poll" in name:
      if "image" in name:
        result.photos.add Photo(url: jsCard{"binding_values", "image_large"}.getImageVal)
  ...
  with jsMedia, js{"extended_entities", "media"}:
    for m in jsMedia:
      case m{"type"}.getStr
      of "photo":
        result.photos.add Photo(url:m{"media_url_https"}.getImageStr, alt:m{"ext_alt_text"}.getStr)
      of "video":
